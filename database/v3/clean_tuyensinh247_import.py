...
def catalog_lookup(
    ...
    # 1. Khớp chính xác & Khớp cắt đuôi
    for candidate in _extract_base_code(program_code):
        entry = uni_cat.get(candidate)
        if entry:
            return entry
    ...
    # 3. Khớp bằng tên ngành
    if major_name:
        score_norm = search_text(major_name).replace(" ", "")
        if score_norm:
            for cat_code, entry in uni_cat.items():
                cat_name = entry.get("major_name", "")
                if cat_name and search_text(cat_name).replace(" ", "") == score_norm:
                    return entry
