...
def crawl_course_info(
    ...
    for item in raw_majors:
        ...
        if program_code not in grouped:
            grouped[program_code] = {
                "major_name":        major_name,
                "quota":             quota,
                "subject_groups":    set(subject_groups),
                "admission_methods": set(admission_methods),
            }
        else:
            g = grouped[program_code]
            if quota is not None and g["quota"] is None:
                g["quota"] = quota
            g["subject_groups"].update(subject_groups)
            g["admission_methods"].update(admission_methods)

    entries: list[CourseCatalogEntry] = []
    for program_code, data in grouped.items():
        entries.append(CourseCatalogEntry(
            university_short_name=uni.short_name,
            program_source_code=program_code,
            major_name=data["major_name"],
            quota=data["quota"],
            admission_methods=sorted(data["admission_methods"]),
            subject_groups=sorted(data["subject_groups"]),
        ))

    logger.info("[%s] course_info: %d ngành", uni.short_name, len(entries))
    return entries
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--info-file",          default=None, help="Path tới university_course_links.txt")
        ...
        with ThreadPoolExecutor(max_workers=max(1, args.workers)) as executor:
            ...
            for future in as_completed(future_map):
                ...
                try:
                    ...
                    entries = []
                catalog_by_uni[uni.short_name] = {e.program_source_code: e for e in entries}
