...
def clean(input_dir: Path, output_dir: Path, universities_file: Path) -> None:
    ...
    for key, meta in sorted(program_meta.items()):
        ...
        if catalog_entry:
            ...
            missing_reason, status, is_active = "Ngành đã khai tử/Không tuyển", "deprecated", "false"

        if not major_name_raw: missing_reason = "Thiếu tên ngành | " + missing_reason

        row_data = {**meta, "program_name": major_name_raw, "quota": quota, "status": status, "is_active": is_active, "base_program_code": base_code, "missing_reason": missing_reason}

        # --- MỚI: TẤT CẢ VỀ MỘT MỐI (COMPLETE) ---
        # Bất kể là canonical hay historical/deprecated, cứ có tên ngành là cho vào Complete hết
        if major_name_raw:
            # Nếu là historical/deprecated thì đánh dấu is_active=false để Frontend không hiển thị mặc định
            # Nhưng vẫn đẩy vào Complete để đại ca phân tích
    ...
    # If any program codes appear in both "complete" and "unmatched", move them
    # into the missing list so they end up in `university_programs_missing.csv`.
    if unmatched_programs:
        unmatched_map = {(r["university_short_name"], r["program_source_code"]): r["reason"] for r in unmatched_programs}
        new_complete: list[dict[str, Any]] = []
        moved = 0
        for r in program_rows_complete:
            key = (r.get("university_short_name"), r.get("program_source_code"))
            if key in unmatched_map:
                missing_reason = unmatched_map[key]
                program_rows_missing.append({
                    "id": r.get("id"),
                    "university_short_name": r.get("university_short_name"),
                    "major_code": r.get("major_code", ""),
                    "program_source_code": r.get("program_source_code"),
                    "program_name": r.get("program_name", ""),
                    "quota": r.get("quota", ""),
                    "missing_reason": missing_reason,
                    "base_program_code": r.get("base_program_code", ""),
                    "source": r.get("source", SOURCE),
                    "source_school_id": r.get("source_school_id", ""),
                    "is_active": r.get("is_active", "false"),
                })
                moved += 1
            else:
                new_complete.append(r)
        program_rows_complete = new_complete
        if moved:
            print(f"Moved {moved} programs from Complete → Missing based on unmatched list")
